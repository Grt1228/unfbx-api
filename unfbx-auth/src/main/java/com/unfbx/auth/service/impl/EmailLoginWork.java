...
public class EmailLoginWork implements IWorker<LoginNameLoginReq, User>, ICallback<LoginNameLoginReq, User> {

    private UserService userService;

    public EmailLoginWork(UserService userService) {
    ...
    public User action(LoginNameLoginReq req, Map<String, WorkerWrapper> allWrappers) {
        User user = this.userService.queryByEmailAndPwd(req.getLoginName(), req.getPassword());
        return user;
    }
